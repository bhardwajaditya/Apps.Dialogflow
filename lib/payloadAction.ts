...
export const  handlePayloadActions = async (app: IApp, read: IRead,  modify: IModify, http: IHttp, rid: string, visitorToken: string, dialogflowMessage: IDialogflowMessage) => {
        ...
        if (action) {
            ...
            if (actionName) {
                if (actionName === ActionIds.PERFORM_HANDOVER) {
                    ...
                    await sendWelcomeEventToDialogFlow(app, read, modify, http, rid, visitorToken, livechatData);
                }
            }
        }
    }
